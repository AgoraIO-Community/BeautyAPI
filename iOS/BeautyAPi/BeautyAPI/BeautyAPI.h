...
typedef NS_ENUM(NSInteger, BeautyPresetMode) {
    ...
    /// Custom beauty parameters, set externally
...
typedef NS_ENUM(NSInteger, MirrorMode) {
    ...
    /// Only local is mirrored, remote is not mirrored, used for video call scenarios, e-commerce live streaming scenarios (to ensure text on signs is readable); in this mode, text stickers will be reversed on one side since local and remote are opposite
    MirrorMode_LOCAL_ONLY = 1,
    /// Only remote is mirrored
    MirrorMode_REMOTE_ONLY= 2,
    /// Neither local nor remote are mirrored, default for rear camera
    MirrorMode_NONE
};
...
// Default mirror for rear camera
@property(nonatomic, assign) MirrorMode backMirror;
@end

@interface BeautyConfig : NSObject
#if __has_include(<AgoraRtcKit/AgoraRtcKit.h>)
// RTC object passed from outside, cannot be null
@property(nonatomic, weak)AgoraRtcEngineKit *rtcEngine;
#endif
// Beauty SDK interface object passed from outside (different for different vendors), cannot be null
...
/**
 * Create and initialize beauty scene API. If registerVideoFrameObserver has been called externally, create must be called after it
 *
...
/**
 * Set camera mirror mode, note that front and rear cameras need to be controlled separately
 *
 *
 * @return 0: Success; non-0: See error codes
 **/
...
/**
 * Local view rendering, mirror issues handled internally
 *
...
/**
 * Destroy beauty scene API.
 * When created with useCustom=true, will call rtcEngine.registerVideoFrameObserver(null) to unbind raw data callback.
 *
