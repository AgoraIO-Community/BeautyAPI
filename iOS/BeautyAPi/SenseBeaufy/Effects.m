...
//    st_mobile_effect_set_param(handle, EFFECT_PARAM_QUATERNION_SMOOTH_FRAME, 5.0); /// X< Set the camera posture smoothing parameters, indicating how many frames to smooth, the larger the delay, the higher, and the weaker the jitter.
...
- (GLuint)processTexture:(GLuint)inputTexture
    ...
    if (handle) {
        ...
        if (CHECK_FLAG(type, EFFECT_CUSTOM_CAMERA_QUATION)) {
            ...
            } else { // Get the default camera quaternim
...
//        _gan_modul_state_change_callback(handle, p_module_info);
    } else if (p_module_info->type == EFFECT_MODULE_SEGMENT) { // Green screen segmentation
