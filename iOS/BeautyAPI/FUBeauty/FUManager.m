...
- (void)setBuauty: (BOOL)isSelected {
    ...
    if (isSelected) {
        ...
        // Default uniform skin smoothing
        beauty.heavyBlur = 0;
        beauty.blurType = 3;
...
- (void)updateBeautyBlurEffect {
    ...
    if ([FURenderKit devicePerformanceLevel] == FUDevicePerformanceLevelHigh) {
        // Set different skin smoothing effects based on face confidence
        ...
        // Set fine skin smoothing effect
        [FURenderKit shareRenderKit].beauty.blurType = 2;
        [FURenderKit shareRenderKit].beauty.blurUseMask = NO;
    }
#endif
...
- (CVPixelBufferRef)processFrame:(CVPixelBufferRef)frame {
    ...
    // By default, faces in the image are always facing up, no need to modify this property when rotating the screen
    ...
    // Must be set if source is from camera capture, otherwise internal detection will be abnormal
    input.renderConfig.isFromFrontCamera = YES;
    // This property indicates whether the system camera has been mirrored: generally frames from front camera are mirrored by default, so this needs to be set. If camera property is not set to mirror, this property does not need to be set.
