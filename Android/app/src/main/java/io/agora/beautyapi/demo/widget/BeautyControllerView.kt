...
import io.agora.beautyapi.demo.databinding.BeautyControllerViewBinding
import io.agora.beautyapi.demo.databinding.BeautyControllerViewItemBinding
import io.agora.beautyapi.demo.databinding.BeautyControllerViewPageBinding

open class BeautyControllerView : FrameLayout {


    private val itemAdapterList =
        mutableListOf<BindingSingleAdapter<ItemInfo, BeautyControllerViewItemBinding>>()

    private val pageAdapter by lazy {
        object : BindingSingleAdapter<PageInfo, BeautyControllerViewPageBinding>() {

            override fun onCreateViewHolder(
                parent: ViewGroup,
                viewType: Int
            ): BindingViewHolder<BeautyControllerViewPageBinding> {
            ...
            override fun onBindViewHolder(
                holder: BindingViewHolder<BeautyControllerViewPageBinding>,
                tabPosition: Int
            ) {
    ...
    val viewBinding by lazy {
        BeautyControllerViewBinding.inflate(LayoutInflater.from(context))
    }
    ...
    private fun createItemAdapter(pageIndex: Int) =
        object : BindingSingleAdapter<ItemInfo, BeautyControllerViewItemBinding>() {

            private var selectedHolder: BindingViewHolder<BeautyControllerViewItemBinding>? = null

            override fun onBindViewHolder(
                holder: BindingViewHolder<BeautyControllerViewItemBinding>,
                position: Int
            ) {
                ...
                val icon = RoundedBitmapDrawableFactory.create(
                    resources,
                    BitmapFactory.decodeResource(resources, itemInfo.icon)
                )
                icon.cornerRadius = 999.0f
                holder.binding.ivIcon.setImageDrawable(icon)

                if (itemInfo.withPadding) {
                    val padding =
                        holder.binding.ivIcon.context.resources.getDimensionPixelSize(R.dimen.beauty_item_padding)
                    holder.binding.ivIcon.setPadding(padding, padding, padding, padding)
                } else {
                    val padding =
                        holder.binding.ivIcon.context.resources.getDimensionPixelSize(R.dimen.beauty_item_padding_background)
                    holder.binding.ivIcon.setPadding(padding, padding, padding, padding)
                }
